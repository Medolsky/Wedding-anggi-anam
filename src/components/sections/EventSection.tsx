...
export function EventSection() {
  ...
  return (
    <section
      id="event"
      data-section="event"
      className="section-event relative py-18 md:py-24 overflow-hidden flex flex-col items-center justify-center text-center bg-[#1c0a08] text-white transform-gpu"
    >
      ...
      <div className="relative z-20 max-w-md mx-auto px-6 w-full text-center flex flex-col items-center justify-center">
        ...
        <AnimatedText delay={0.1} variant="scaleUp" className="w-full text-center">
          <h2
            className="text-2xl md:text-3xl text-center mb-8 font-serif text-white drop-shadow-sm leading-snug"
            ...
            <StaggerItem key={event.id}>
              ...
              >
                ...
                <div className="relative h-36 w-full overflow-hidden">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#1c0a08] via-black/30 to-transparent" />
                  <div className="absolute bottom-2.5 left-3.5 right-3.5 flex items-center justify-between text-white">
                    <span className="text-sm md:text-base font-bold tracking-wide font-serif text-[#f3e5ab]">
                      {event.title}
                    </span>
                    <span className="text-[9px] bg-[#d4af37] text-black font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                      {event.type}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-4 text-center flex flex-col items-center justify-center">
                  <div className="space-y-2 mb-4 text-xs w-full text-center">
                    ...
                    {/* Location */}
                    <div className="flex flex-col items-center justify-center gap-0.5 pt-1.5 border-t border-white/10">
                      <div className="flex items-center gap-1.5">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#d4af37" strokeWidth="2">
                          <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
                          <circle cx="12" cy="10" r="3" />
                        </svg>
                        <p className="font-bold text-xs md:text-sm text-white">{event.venue}</p>
                      </div>
                      <p className="text-white/80 text-[10px] max-w-xs text-center leading-relaxed">{event.address}</p>
                    </div>

                    {event.dressCode && (
                      <div className="pt-1.5 border-t border-[#d4af37]/20 text-center">
                        <span className="text-[10px] text-white/80">
                          Dress Code: <strong className="text-[#d4af37] font-semibold">{event.dressCode}</strong>
                        </span>
                      </div>
                    )}
                  </div>
