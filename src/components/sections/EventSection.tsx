          ...
          {weddingData.events.map((event) => {
            ...
            return (
              <StaggerItem key={ev.id}>
                <motion.div
                  className="gold-card-pro overflow-hidden shadow-2xl border border-[#d4af37]/60 text-center w-full rounded-2xl transform-gpu"
                  whileHover={{ y: -2 }}
                ...
                >
                  ...
                  <div className="relative h-44 md:h-48 w-full overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#1a1815]/80 via-black/20 to-transparent" />
                    <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between text-white">
                      <span className="text-base md:text-lg font-bold tracking-wide font-serif text-[#f7ebbf] drop-shadow-md">
                        {ev.title}
                      </span>
                      <span className="text-[10px] bg-gradient-to-r from-[#d4af37] to-[#b8860b] text-white font-extrabold px-3 py-1 rounded-full uppercase tracking-wider shadow-md">
                        {ev.type}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-4 text-center flex flex-col items-center justify-center">
                    <div className="space-y-3 mb-4 text-xs w-full text-center">
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="font-bold text-sm text-[#2a2723]">{ev.date}</span>
                      </div>
                      ...
                      {ev.akadTime && ev.resepsiTime ? (
                        <div className="grid grid-cols-2 gap-2 my-2 w-full">
                          <div className="bg-[#b8860b]/10 rounded-xl p-2.5 text-center border border-[#b8860b]/25 shadow-sm">
                            <p className="text-[10px] uppercase font-bold text-[#b8860b] tracking-wider mb-0.5">
                              Akad Nikah
                            </p>
                            <p className="text-xs font-semibold text-[#2a2723]">
                              {ev.akadTime}
                            </p>
                          </div>
                          <div className="bg-[#b8860b]/10 rounded-xl p-2.5 text-center border border-[#b8860b]/25 shadow-sm">
                      ...
                      <div className="flex flex-col items-center justify-center gap-0.5 pt-1.5 border-t border-[#d4af37]/20">
                        <div className="flex items-center gap-1.5">
                          ...
                          <p className="font-bold text-xs md:text-sm text-[#2a2723]">{ev.venue}</p>
                        </div>
                        <p className="text-[#66615c] text-[10px] max-w-xs text-center leading-relaxed">{ev.address}</p>
                      </div>

                      {ev.dressCode && (
                        <div className="pt-1.5 border-t border-[#d4af37]/20 text-center">
                          <span className="text-[10px] text-[#66615c]">
                            Dress Code: <strong className="text-[#b8860b] font-semibold">{ev.dressCode}</strong>
                          </span>
                        </div>
                      )}
                    </div>
