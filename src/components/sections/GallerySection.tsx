...
export function GallerySection() {
  ...
  return (
    <section
      ...
      className="section-gallery relative py-20 md:py-28 overflow-hidden flex flex-col items-center justify-center text-center bg-[#FAF8F5] text-[#1A1815]"
    >
      ...
      <div className="relative z-20 max-w-lg mx-auto px-6 w-full text-center flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-2 gap-3.5 w-full">
          {gallery.map((item, index) => (
            <motion.div
              key={item.id}
              className={`relative rounded-2xl overflow-hidden cursor-pointer group shadow-2xl border-2 border-[#C8A96B] bg-white
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/35 transition-colors duration-300 flex items-center justify-center">
                <motion.div
                  className="text-[#C8A96B] opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white/90 p-2.5 rounded-full border border-[#C8A96B] shadow-md"
                  initial={false}
                >
                  <svg
                    width="18"
