"use client";

import { motion, AnimatePresence } from "framer-motion";
import { useState, useEffect } from "react";

export function WelcomeCover({
  guestName,
  onOpen,
}: {
  guestName?: string;
  onOpen?: () => void;
}) {
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    // Ensure button is visible after 2.7s even if video event doesn't trigger
    const timer = setTimeout(() => {
      setIsVisible(true);
    }, 2700);
    return () => clearTimeout(timer);
  }, []);

  const handleTimeUpdate = (e: React.SyntheticEvent<HTMLVideoElement>) => {
    const time = e.currentTarget.currentTime;
    if (time >= 2.7) {
      if (!isVisible) setIsVisible(true);
    }
  };

  return (
    <div className="absolute inset-0 overflow-hidden">
      {/* Background Video — welcome1.mp4 */}
      <video
        autoPlay
        loop
        muted
        playsInline
        onTimeUpdate={handleTimeUpdate}
        className="absolute inset-0 w-full h-full object-cover transform-gpu scale-105"
      >
        <source src="/image/welcome1.mp4" type="video/mp4" />
      </video>

      {/* Floating Gold Particles */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-10">
        {Array.from({ length: 10 }).map((_, i) => (
          <motion.div
            key={i}
            className="absolute w-1.5 h-1.5 rounded-full bg-[#C8A96B] shadow-[0_0_8px_#C8A96B]"
            style={{
              left: `${10 + i * 9}%`,
              top: `${15 + (i % 4) * 20}%`,
              opacity: 0.3 + (i % 3) * 0.15,
            }}
            animate={{
              y: [0, -40, 0],
              x: [0, (i % 2 ? 15 : -15), 0],
              opacity: [0.2, 0.6, 0.2],
            }}
            transition={{
              duration: 5 + i * 1.1,
              repeat: Infinity,
              ease: "easeInOut",
              delay: i * 0.4,
            }}
          />
        ))}
      </div>

      {guestName && onOpen && (
        <WelcomeContent
          guestName={guestName}
          onOpen={onOpen}
          isVisible={isVisible}
        />
      )}
    </div>
  );
}

export function WelcomeContent({
  guestName,
  onOpen,
  isVisible = true,
}: {
  guestName: string;
  onOpen: () => void;
  isVisible?: boolean;
}) {
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div
          className="absolute inset-x-0 top-[81%] -translate-y-1/2 z-20 flex flex-col items-center justify-center px-6 text-center pointer-events-auto"
          initial={{ opacity: 0, y: 20, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          exit={{ opacity: 0, y: 15, scale: 0.95 }}
          transition={{ duration: 0.8, ease: "easeInOut" }}
        >
          {/* Guest Name Card — Locked in empty space below 10 Oktober 2026 */}
          <div className="mb-3 px-5 py-3 bg-black/45 backdrop-blur-md border border-[#C8A96B] shadow-[0_8px_32px_rgba(0,0,0,0.5)] max-w-[240px] w-full text-center rounded-2xl">
            <p
              className="text-[9.5px] uppercase tracking-[2.5px] text-[#E0C98F] font-extrabold mb-0.5 leading-none text-center drop-shadow-xs"
              style={{ fontFamily: "var(--font-body)" }}
            >
              KEPADA YTH.
            </p>
            <p
              className="text-base font-bold tracking-wide text-[#F5F1E8] text-center font-serif leading-snug drop-shadow-sm"
              style={{ fontFamily: "var(--font-heading)" }}
            >
              {guestName}
            </p>
          </div>

          {/* Open Button */}
          <motion.button
            className="btn-invitation group text-[11px] py-2.5 px-6 shadow-xl cursor-pointer"
            onClick={onOpen}
            whileHover={{ scale: 1.04 }}
            whileTap={{ scale: 0.96 }}
          >
            <span className="flex items-center gap-1.5 font-extrabold tracking-widest">
              <svg
                width="13"
                height="13"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
                <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
                <polyline points="22,6 12,13 2,6" />
              </svg>
              BUKA UNDANGAN
            </span>
          </motion.button>
        </motion.div>
      )}
    </AnimatePresence>
  );
}
